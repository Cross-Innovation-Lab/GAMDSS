...
class Cas3c4DataSet(data.Dataset):
    def __init__(self, raf_path, phase,num_loso, transform = None, basic_aug = False, transform_norm=None):
        ...
        df = pd.read_excel(r'D:\PycharmData\CAS(ME)3\cas(me)3_part_A_ME_label_JpgIndex_v2.xlsx', usecols=[0,1,2,3,4,5,7,8])
        ...
        differ_num = dataset.iloc[:, DIFFER_COLUMN].values
        ...
        for (f,sub,onset,apex,offset,label_all,label_au,differ_all) in zip(File_names,Subject,Onset_num,Apex_num,Offset_num,Label_all,Label_au,differ_num):
            ...
            if label_all == 'anger' or label_all == 'disgust' or label_all == 'fear' or label_all == 'happy' or label_all == 'sad' or label_all == 'surprise' or label_all == 'others':
                ...
                self.differ_all.append(differ_all)
                if label_all == 'happy':
    ...
    def __getitem__(self, idx):
        ...
        if self.phase == 'train':
            ...
            differ = self.differ_all[idx]
            if differ == 1:
                # print(sub, f, onset, apex, offset)
                on0, apex0, offset = casme3_full_difference(int(onset), int(apex), int(offset), self.raf_path, sub, f)
            elif differ == 0:
                on0 = str(onset)
                apex0 = str(apex)
                offset = str(offset)
        ...
        path_off0 = os.path.join(self.raf_path, sub, f, offset)
