...
from GAMDSS import *
from cas3.dataset_c4 import Cas3c4DataSet
from cas3.dataset_c7 import Cas3DataSet
from models.model import *
import seaborn as sns
import matplotlib.pyplot as plt

sys.stdout = Logger("cas3_c4.log")
...
def run_training(raf_path):
    ...
    acc_pred_ALL = torch.zeros(4)
    acc_label_ALL = torch.zeros(4)
    acc_TP_ALL = torch.zeros(4)

    pre_ALL = []
    labels_ALL = []

    for subj in LOSO:
        train_dataset = Cas3c4DataSet(raf_path, phase='train', num_loso=subj, transform=data_transforms,
                                   basic_aug=True, transform_norm=data_transforms_norm)
        val_dataset = Cas3c4DataSet(raf_path, phase='test', num_loso=subj, transform=data_transforms_val)
    ...
    # #cm = cm.T
    label_name = ['Positive', 'Surprise', 'Others', 'Negative']
    # 可视化混淆矩阵
