...
def samm_full_difference(onset, apex, offset, file_path=None, sub=None, f=None):
    onset = onset
    apex = apex
    offset = offset
    diff_dict = {}
    ...
    onset_path = path + '/' + '%03d' % int(sub) + '_' + str(onset) + '.jpg'
    apex_path = path + '/' + '%03d' % int(sub) + '_' + str(apex) + '.jpg'
    offset_path = path + '/' + '%03d' % int(sub) + '_' + str(offset) + '.jpg'
    onset_img = cv2.imread(onset_path)
    apex_img = cv2.imread(apex_path)
    offset_img = cv2.imread(offset_path)
    stand = np.linalg.norm(apex_img - onset_img)
    diff_dict[(onset, apex)] = stand
    for index1 in range(1, ran+1):
        onset0 = onset + index1
    ...
    apex_index_path = path + '/' + '%03d' % int(sub) + '_' + str(apex_index) + '.jpg'
    apex_index_img = cv2.imread(apex_index_path)
    stand1 = np.linalg.norm(apex_index_img - offset_img)
    after_diff_dict[(apex_index, offset)] = stand1
    for index4 in range(1, ran_off + 1):
        offset0 = offset - index4
        offset0_path = path + '/' + '%03d' % int(sub) + '_' + str(offset0) + '.jpg'
        offset0_img = cv2.imread(offset0_path)
        diff4 = np.linalg.norm(apex_index_img - offset0_img)
        after_diff_dict[(apex_index, offset0)] = diff4
    after_max_diff_key = max(after_diff_dict, key=after_diff_dict.get)

    _, offset_index = after_max_diff_key

    return onset_index, apex_index, offset_index

def samm_min_max_difference(onset, apex, offset, file_path=None, sub=None, f=None):
    onset = onset
    apex = apex
    offset = offset
    diff_dict = {}
    after_diff_dict = {}
    ran = int(0.32 * (apex - onset))
    ran_off = int(0.52 * (offset - apex))
    #ran = min(ran, 3)
    path = os.path.join(file_path, '%03d' % int(sub), f)
    onset_path = path + '/' + '%03d' % int(sub) + '_' + str(onset) + '.jpg'
    apex_path = path + '/' + '%03d' % int(sub) + '_' + str(apex) + '.jpg'
    ...
    #         diff2 = np.linalg.norm(apex0_img - onset0_img)
    #         diff_dict[(onset0, apex0)] = diff2
    for index3 in range(1, ran_off+1):
        apex1 = apex + index3
        apex1_path = path + '/' + '%03d' % int(sub) + '_' + str(apex1) + '.jpg'
        apex1_img = cv2.imread(apex1_path)
        diff3 = np.linalg.norm(apex1_img - onset_img)
...
def samm_max_difference(onset, apex, offset, file_path=None, sub=None, f=None):
    onset = onset
    apex = apex
    offset = offset
    diff_dict = {}
    ran = int(0.32 * (apex - onset))
    ran_off = int(0.48 * (offset - apex))
    path = os.path.join(file_path, '%03d' % int(sub), f)
    onset_path = path + '/' + '%03d' % int(sub) + '_' + str(onset) + '.jpg'
    apex_path = path + '/' + '%03d' % int(sub) + '_' + str(apex) + '.jpg'
    offset_path = path + '/' + '%03d' % int(sub) + '_' + str(offset) + '.jpg'
    ...
    for index1 in range(1, ran+1):
        ...
        onset0_path = path + '/' + '%03d' % int(sub) + '_' + str(onset0) + '.jpg'
        onset0_img = cv2.imread(onset0_path)
        for index2 in range(0, ran_off+1):
            apex0 = apex + index2
            apex0_path = path + '/' + '%03d' % int(sub) + '_' + str(apex0) + '.jpg'
            apex0_img = cv2.imread(apex0_path)
            diff2 = np.linalg.norm(apex0_img - onset0_img)
...
def casme3_full_difference(onset, apex, offset, file_path=None, sub=None, f=None):
    onset = onset
    apex = apex
    offset = offset
    diff_dict = {}
    ...
    onset_path = path + '/' + str(onset) + '.jpg'
    apex_path = path + '/' + str(apex) + '.jpg'
    offset_path = path + '/' + str(offset) + '.jpg'
    onset_img = cv2.imread(onset_path)
    apex_img = cv2.imread(apex_path)
    offset_img = cv2.imread(offset_path)
    offset_img = cv2.resize(offset_img, (apex_img.shape[1], apex_img.shape[0]))
    stand = np.linalg.norm(apex_img - onset_img)
    diff_dict[(onset, apex)] = stand
    for index1 in range(1, ran+1):
        onset0 = onset + index1
    ...
    apex_index_path = path + '/' + str(apex_index) + '.jpg'
    apex_index_img = cv2.imread(apex_index_path)
    stand1 = np.linalg.norm(apex_index_img - offset_img)
    after_diff_dict[(apex_index, offset)] = stand1
    for index4 in range(1, ran_off + 1):
        offset0 = offset - index4
        offset0_path = path + '/' + str(offset0) + '.jpg'
        offset0_img = cv2.imread(offset0_path)
        offset0_img = cv2.resize(offset0_img, (apex_img.shape[1], apex_img.shape[0]))
        diff4 = np.linalg.norm(apex_index_img - offset0_img)
        after_diff_dict[(apex_index, offset0)] = diff4
    after_max_diff_key = max(after_diff_dict, key=after_diff_dict.get)
...
def flow_full_difference(onset, apex, offset, file_path=None, sub=None, f=None):
    onset = onset
    apex = apex
    offset = offset
    diff_dict = {}
    ...
    onset_path = path + '/reg_img' + str(onset) + '.jpg'
    apex_path = path + '/reg_img' + str(apex) + '.jpg'
    offset_path = path + '/reg_img' + str(offset) + '.jpg'
    onset_img = cv2.imread(onset_path)
    apex_img = cv2.imread(apex_path)
    offset_img = cv2.imread(offset_path)
    stand = np.linalg.norm(apex_img - onset_img)
    ...
    onset_index, apex_index = max_diff_key
    apex_index_path = path + '/reg_img' + str(apex_index) + '.jpg'
    apex_index_img = cv2.imread(apex_index_path)
    stand1 = np.linalg.norm(apex_index_img - offset_img)
    after_diff_dict[(apex_index, offset)] = stand1
    for index4 in range(1, ran_off + 1):
        offset0 = offset - index4
        offset0_path = path + '/reg_img' + str(offset0) + '.jpg'
        offset0_img = cv2.imread(offset0_path)
        diff4 = np.linalg.norm(apex_index_img - offset0_img)
        after_diff_dict[(apex_index, offset0)] = diff4
        # for index5 in range(1, ran_off + 1):
        ...
        #     apex2_path = path + '/reg_img' + str(apex2) + '.jpg'
        #     apex2_img = cv2.imread(apex2_path)
        #     diff5 = np.linalg.norm(apex2_img - offset0_img)
        #     after_diff_dict[(apex0, offset0)] = diff5
    # for index6 in range(1, ran_off + 1):
    #     apex3 = apex + index6
    #     apex3_path = path + '/reg_img' + str(apex3) + '.jpg'
    #     apex3_img = cv2.imread(apex3_path)
    #     diff6 = np.linalg.norm(apex3_img - offset_img)
    #     after_diff_dict[(apex3, offset)] = diff6
    after_max_diff_key = max(after_diff_dict, key=after_diff_dict.get)
