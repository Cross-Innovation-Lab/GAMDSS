...
from GAMDSS import *
from cas3.dataset_c7 import Cas3DataSet
...
def run_training(raf_path):
    ...
    acc_pred_ALL = torch.zeros(7)
    acc_label_ALL = torch.zeros(7)
    acc_TP_ALL = torch.zeros(7)

    pre_ALL = []
    labels_ALL = []

    for subj in LOSO:
        train_dataset = Cas3DataSet(raf_path, phase='train', num_loso=subj, transform=data_transforms,
                                   basic_aug=True, transform_norm=data_transforms_norm)
        val_dataset = Cas3DataSet(raf_path, phase='test', num_loso=subj, transform=data_transforms_val)
        ...
        acc_pos_label = torch.zeros(7)
        acc_TP = torch.zeros(7)
        pred_loso = []
        label_loso = []

        ##model initialization
        net_all = BDRT(num_classes=7) ## BDRT or DRT or ADRT or other
        ...
        for i in range(1, 200):
            ...
            for batch_i, (image_on0, image_apex0, image_off0, label_all) in enumerate(train_loader):
                iter_cnt += 1
                ...
                ##train MMNet
                ALL, s = net_all(image_on0, image_apex0, image_off0)

                loss_all = criterion(ALL, label_all)

                loss_s = criterion(s, label_all)
                loss_all = loss_all + loss_s
                ...
                t_features_list.append(ALL.detach().cpu().numpy())
                t_labels_list.append(label_all.detach().cpu().numpy())
            ...
            with torch.no_grad():
                ...
                for batch_i, (image_on0, image_apex0, image_off0, label_all) in enumerate(val_loader):
                ...
                if bingo_cnt > max_corr:
                    max_corr = bingo_cnt
                    acc_pos_label = pos_label
                    acc_pos_pred = pos_pred
                    acc_TP = TP
                    pred_loso = preds
                    label_loso = labels
                if AVG_F1 >= max_f1:
                ...
                if acc==1.:
                    ...
                    break
            if i == 199:
                features = np.concatenate(t_features_list, axis=0)
                labels = np.concatenate(t_labels_list, axis=0)
                ...
                # 自定义颜色列表
                colors = ['red', 'green', 'blue', 'pink', 'orange', 'yellow', 'brown']  # 根据实际情况修改颜色
                label_names = ['Anger', 'Disgust', 'Fear', 'Happy', 'Sad', 'Surprise', 'Other']  # 根据实际情况修改标签名称

                # 绘制t-SNE图
                plt.figure(figsize=(10, 8))
                for ind1, label in enumerate(np.unique(labels)):
                    indices = np.where(labels == label)
                    plt.scatter(features_2d[indices, 0], features_2d[indices, 1], c=colors[ind1],
                                label=label_names[ind1],
                                s=50)  # 图中点的大小

                # 创建图例的句柄
                legend_handles = [
                    plt.Line2D([0], [0], marker='o', color='w', label=label, markerfacecolor=colors[ind2],
                               markersize=10)
                    for ind2, label in enumerate(label_names)]

                # 添加图例
                plt.legend(handles=legend_handles, loc='upper right')
                plt.savefig(f'cas2_c5_full_{i}.png')  # 使用f-string格式化文件名
                plt.show()
        ...
        acc_label_ALL = acc_label_ALL + acc_pos_label
        acc_pred_ALL = acc_pred_ALL + acc_pos_pred
        acc_TP_ALL = acc_TP_ALL + acc_TP
        ...
        SUM_recall = 0
        for index in range(7):
            #print("[%s:] TP: %.4f LAB: %.4f PRE:%.4f" % (index, acc_TP_ALL[index], acc_pred_ALL[index], acc_label_ALL[index]))
    ...
    # for index in range(7):
    #     print("[%s:] TP: %.4f LAB: %.4f PRE:%.4f" % (index, acc_TP_ALL[index], acc_pred_ALL[index], acc_label_ALL[index]))
    #     print("[%s] ACC:%.4f" % (index, acc_TP_ALL[index]/acc_pred_ALL[index]))
    # # 生成混淆矩阵
    # cm = confusion_matrix(labels_ALL, pre_ALL)
    # row_sums = np.sum(cm, axis=1)
    # cm = (cm / row_sums[:, np.newaxis])*100
    # # #cm = cm.T
    # label_name = ['anger', 'disgust', 'fear', 'happy', 'sad', 'surprise', 'others']
    # # 可视化混淆矩阵
    # plt.figure(figsize=(10, 8))
...
if __name__ == "__main__":
    seed = 2
    raf_path = r'D:\PycharmData\CAS(ME)3\part_A\full_cropped'
    seed_torch(seed)
