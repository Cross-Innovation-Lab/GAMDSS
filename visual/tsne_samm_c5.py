...
from GAMDSS import *
from models.model import BDRT
from samm.dataset_c5 import sammDataSet

# sys.stdout = Logger("full_samm_c5.log")
...
def run_training(raf_path):
    ...
    for subj in LOSO:
        train_dataset = sammDataSet(raf_path, phase='train', num_loso=subj, transform=data_transforms,
                                   basic_aug=True, transform_norm=data_transforms_norm)
        val_dataset = sammDataSet(raf_path, phase='test', num_loso=subj, transform=data_transforms_val)
        ...
        net_all = BDRT(num_classes=5) ## BDRT or DRT or ADRT or other
        ...
        for i in range(1, 200):
            ...
            with torch.no_grad():
                ...
                # net_au.eval()
                for batch_i, (
                image_on0, image_apex0, image_off0, label_all) in enumerate(val_loader):
                    batch_sz = image_on0.size(0)
                    b, c, h, w = image_on0.shape
            ...
            if i == 199:
                ...
                # 自定义颜色列表
                colors = ['red', 'green', 'blue', 'yellow', 'brown']  # 根据实际情况修改颜色
                label_names = ['Anger', 'Disgust',  'Fear', 'Contempt', 'Other']  # 根据实际情况修改标签名称
                ...
                plt.savefig(f'samm_c5_full_{i}.png')  # 使用f-string格式化文件名
                plt.show()
