...
from GAMDSS import flow_full_difference


class res_Cas2c3DataSet(data.Dataset):
    def __init__(self, raf_path, phase,num_loso, transform = None, basic_aug = False, transform_norm=None):
        ...
        LABEL_AU_COLUMN = 5
        LABEL_ALL_COLUMN = 6
        df = pd.read_excel(os.path.join(self.raf_path, 'CASME2-coding-20241216.xlsx'),usecols=[0,1,3,4,5,7,8])
    ...
    def __getitem__(self, idx):
        ...
        if self.phase == 'train':
            ...
            # on0, apex0, off0 = flow_full_difference(int(onset), int(apex), int(offset), self.raf_path, sub, f)
            # on0 = str(on0)
            # apex0 = str(apex0)
            # offset = str(off0)
            on0 = str(onset)
        ...
        path_off0 = os.path.join(self.raf_path, 'Cropped/', 'sub%02d' % int(sub), f, offset)
